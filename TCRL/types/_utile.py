...
def make_id(self):
    self.__id__ = int(datetime.now().strftime("%Y%m%d%H%M%S%f")+str(random.randint(0, 1000)))
...
def get_vars(self, obj_params, params, obj_kwargs, kwargs, vars_objets):
    ...
    if len(obj_params) == len(params):
        ...
        raise TypeError("%s() manque %s arguments: %s"%(self.nom, len(args), ", ".join(map(repr, args))))

    else:
        raise TypeError("%s() à %s arguments, mais %s sont donnés: %s"%(self.nom, len(obj_params), len(params), ", ".join(map(repr, params))))
