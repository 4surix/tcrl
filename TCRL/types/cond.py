...
class result:
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, (int, pos, neg)):
            return self.retours[item]
        elif isinstance(item, (str, bytes)):
            retour_act = self.actions[item]
            if len(retour_act) == 1:
                return retour_act[0]
            else:
                return tuple(retour_act)

    def __setitem__(self, action, retour):

        if action not in self.actions:
            self.actions[action] = []
        self.actions[action].append(retour)
...
class cond:
    ...
    def append(self, obj):
        ...
        if not self.if_ and not any([self.params, self.clones, self.kwargs, self.comparaison, self.actions[True], self.actions[False]]) and isinstance(obj, tuple):
            ...
            if obj == 'if':
                ...
                if self.type == 'cond':
                    self.comparaison.append(obj)

                elif self.type == 'up':
                    self.remonter.append(obj)

                elif self.type == 'acts':
                    if isinstance(obj, py_str) and obj.replace('~', '') in noms_actions:
                        self.actions[True].append([obj, []])
                    else:
                        self.actions[True][-1][-1].append(obj)
                    ...
                    if isinstance(obj, py_str) and obj.replace('~', '') in noms_actions:
                        self.actions[False].append([obj, []])
                    else:
                        self.actions[False][-1][-1].append(obj)


    def __str__(self):
        return ':%s:'%self.comparaison
    def __repr__(self):
        return ':%s:'%self.comparaison
    ...
    def __call__(self, params, kwargs, vars_objets):
        variables = get_vars(self, self.params, params, self.kwargs, kwargs, vars_objets)

        act_variables = variables
        act_defauts = self.bloc.__defauts__
        act_decode = self.bloc.__decode__
        ...
        while True:
            ...
            if nbr_boucle > 1000:
                raise Exception('Module : %s, Cond : %s, Text : Le nombre de remontée à atteint le max (1000)'%(self.bloc.__name__, self.nom))

            if self.comparaison:
                status = _analyse_comparaison(self.comparaison, *act_utile)
            else:
                status = True
            ...
            if actions:
                ...
                for action, values in actions:
                    ...
                    if action == 'aff':
                        ...
                        """
                        value = values[0]
                        raise Exception('Module : %s, Cond : %s, Text : %s'%(self.bloc.__name__, self.nom, value))
                ...
                if len(self.remonter) == 1:
                    if py_bool(self.remonter[0]) == status:
                        continue
                elif self.remonter:
                    if _analyse_comparaison(self.remonter, *act_utile):
                        continue
